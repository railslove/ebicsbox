  ...
  module Entities
    class Subscriber < Grape::Entity
      ...
      expose(:_links, documentation: { type: "Hash", desc: "Links to resources" }) do |subscriber, options|
        {
          self: Box.configuration.app_url + "/management/#{subscriber.account.iban}/subscribers/#{subscriber.id}",
          ini_letter: Box.configuration.app_url + "/management/users/#{subscriber.id}/ini_letter",
          account: Box.configuration.app_url + "/management/#{subscriber.account.iban}",
