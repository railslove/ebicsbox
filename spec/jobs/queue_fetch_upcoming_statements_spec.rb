...
module Box
  module Jobs
    RSpec.describe QueueFetchUpcomingStatements do
      subject(:job) { described_class.new }

      describe '#perform' do
        it 'queues processing status job for each account_id' do
          expect { job.perform([1, 2, 3]) }.to change(Box::Jobs::FetchUpcomingStatements.jobs, :size).by(3)
        end

        it 'queues with account id' do
          job.perform([1337, 4711])

          queued_jobs = Box::Jobs::FetchUpcomingStatements.jobs
